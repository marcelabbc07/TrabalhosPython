...
class CadastroCliente:
    def __init__(self):
        self.lista=self.ler_arquivo()
    def ler_arquivo(self,arquivo):
        arquivo=open('Aula25/clientes.txt', 'r')
        self.lista=[]
        for pessoas in arquivo:
            pessoa=pessoa.strip().split(';')
            dicionario={'codigo':pessoa[0],'nome':pessoa[1],'idade':pessoa[2],'telefone':pessoa[3],'email':pessoa[4],'endereco':pessoa[5]}
